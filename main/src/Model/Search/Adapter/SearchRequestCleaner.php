...
class SearchRequestCleaner extends \Magento\Framework\Search\Request\Cleaner
{
    /**
     * @var array
     */
     ...
     * Changed in order to reflect arrays in ranges
     *
    ...
    private function cleanFilter($filterName)
    {
        ...
        switch ($filter['type']) {
            ...
            case FilterInterface::TYPE_RANGE:
                $keys = ['from', 'to'];
                foreach ($keys as $key) {
                    if (isset($filter[$key]) && (!is_array($filter[$key])) &&  preg_match('/^\$(.+)\$$/si', $filter[$key])) {
