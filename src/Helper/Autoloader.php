 ...
 */
class Integer\Net\Solr\Helper\Autoloader
{
    ...
    {
        if ($this->_configScopeConfigInterface->isSetFlag('integernet_solr/dev/register_autoloader', \Magento\Store\Model\ScopeInterface::SCOPE_STORE)) {
            $libBaseDir = $this->_configScopeConfigInterface->getValue('integernet_solr/dev/autoloader_basepath', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
            if ($libBaseDir[0] !== '/') {
                $libBaseDir = Mage::getBaseDir() . DIRECTORY_SEPARATOR . $libBaseDir;
            }
